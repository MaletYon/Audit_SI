...
A & \texttt{DOMAINE} &
  ...
  sert à la consolidation dans la synthèse). \\ \hline
...
C & \texttt{OBJECTIF DE CONTRÔLE} &
  ...
  (\emph{S'assurer que\ldots}, \emph{Vérifier que\ldots}). \\ \hline
...
E & \texttt{ÉLÉMENT REQUIS} &
  Document ou preuve à collecter (niveau~5). \\ \hline

F & \texttt{QUESTION D'ÉVALUATION} &
  Question posée sur le terrain (niveau~6), appelant une réponse
  trichotomique~: \texttt{O}~/ \texttt{N}~/ \texttt{PAS}. \\ \hline
...
G & \texttt{RÉPONSE} &
  ...
  toutes les autres colonnes calculées en dépendent. \\ \hline

H & \texttt{RISQUE} &
  Formulation du risque associé à une réponse négative (niveau~7).
  Affiché en rouge automatiquement si la réponse en col.~G est~N. \\ \hline

I & \texttt{CONSÉQUENCE} &
  Impact organisationnel de la réalisation du risque (niveau~8). \\ \hline
...
J & \texttt{CRITICITÉ} &
  ...
  Exprimé sur une échelle~1--4 ou 1--9 selon le référentiel retenu. \\ \hline
...
N & \texttt{COMMENTAIRE} &
  ...
  contradictoires, interlocuteur non disponible). \\ \hline
...
O & \texttt{ENTROPIE} &
  ...
  une zone grise à fort niveau d'incertitude. \\ \hline
